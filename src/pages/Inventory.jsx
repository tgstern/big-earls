...
export default function Inventory() {
  ...
  function applyForFinancing() {
    ...
    pendo.track("financing_application_submitted", {
      filteredType: type,
      vehiclesDisplayed: shown.length,
  ...
  function handleFilterChange(e) {
    ...
    const resultsCount =
      newType === "All"
        ? vehicles.length
        : vehicles.filter((v) => v.type === newType).length;
    ...
    <section>
      ...
      <div className="toolbar">
        ...
        <span className="count">
          {shown.length} of {vehicles.length} fine pre-owned units
        </span>
      </div>
        ...
        <tbody>
          {shown.map((v) => (
            <tr key={v.id} id={`row-${v.id}`}>
              ...
              <td>
                {v.badge && <span className="badge-chip">{v.badge}</span>}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      <div className="financing">
        ...
        >
          {approved
            ...
            : "APPLY FOR FINANCING — EVERYBODY'S APPROVED!"}
        </button>
      </div>
    </section>
  );
